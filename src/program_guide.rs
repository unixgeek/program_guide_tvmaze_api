...
pub struct Database {
    ...
    delete_episodes_by_tvmaze_id: Statement,
    insert_episodes_for_tvmaze_id: Statement,
}
...
impl Database {
    pub fn new(url: String) -> Result<Self, Error> {
        ...
        let get_all_programs = connection.prep(
            "SELECT tvmaze_id, name, url, network, UNIX_TIMESTAMP(last_update) AS last_update FROM program")?;

        let get_program_by_tvmaze_id = connection.prep(
            "SELECT tvmaze_id, name, url, network, UNIX_TIMESTAMP(last_update) AS last_update FROM program WHERE tvmaze_id = ?")?;
        ...
        let get_episode_by_episode_number = connection.prep(
            "SELECT tvmaze_id, season, number, DATE_FORMAT(original_air_date, '%Y-%m-%d'), title, summary_url FROM episode WHERE tvmaze_id = ? AND season = ? AND number = ?")?;

        let delete_episodes_by_tvmaze_id = connection.prep(
            "DELETE FROM episode WHERE tvmaze_id = ?")?;

        let insert_episodes_for_tvmaze_id = connection.prep(
            "INSERT INTO episode (tvmaze_id, season, number, original_air_date, title, summary_url) VALUES (?, ?, ?, STR_TO_DATE(?, '%Y-%m-%d'), ?, ?)")?;
        ...
        Ok(
            Self {
                ...
                delete_episodes_by_tvmaze_id,
                insert_episodes_for_tvmaze_id,
            }
        )
    }
    ...
    pub fn get_program_by_tvmaze_id(&mut self, id: u32) -> Result<Option<Program>, Error> {
        let mut list = self.connection.exec_map(
            ...
            |(tvmaze_id, name, url, network, last_update)| {
                Program { tvmaze_id, name, url, network, last_update }
            },
        )?;
    ...
    pub fn get_episode_by_episode_number(&mut self, id: u32, season: u8, number: u8) -> Result<Option<Episode>, Error> {
        let mut list = self.connection.exec_map(
            ...
            |(tvmaze_id, season, number, original_air_date, title, summary_url)| {
                Episode { tvmaze_id, season, number, original_air_date, title, summary_url }
            },
        )?;
    ...
    pub fn delete_episodes_by_program_id(&mut self, id: u32) -> Result<u64, Error> {
        let _result: Vec<u8> = self.connection.exec(&self.delete_episodes_by_tvmaze_id, (id, ))?;
    ...
    pub fn insert_episodes_by_program_id(&mut self, episodes: Vec<Episode>) -> Result<u64, Error> {
        ...
        for episode in episodes {
            let _result: Vec<u8> = self.connection.exec(&self.insert_episodes_for_tvmaze_id, (episode.tvmaze_id, episode.season, episode.number, episode.original_air_date, episode.title, episode.summary_url))?;
